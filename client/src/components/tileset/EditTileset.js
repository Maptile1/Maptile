...
const EditTileset = (props) => {
  ...
  useEffect(() => {
    const getTileset = async () => {
      ...
      if (
        tilesetdata.data.tileset.tileset_data.length === 0 &&
        localStorage.getItem("imgData") === null
      ) {
        tilesetdata.data.tileset.tileset_data.push(
          initTileset(tilesetdata.data.tileset)
        );
      } else if (
        localStorage.getItem("imgData") !== null &&
        tilesetdata.data.tileset.tileset_data.length === 0
  ...
  useEffect(() => {
    if (zoomLevel === 1 && showDividers === false && download) {
      ...
      stageRef.current.toBlob().then((img) => {
        ...
        Axios.post(
          "https://maptile1.herokuapp.com/tileset/image/" + tileset._id,
          formData
        )
      ...
      for (let i = 0; i < tileset.tileset_height; i++) {
        ...
        for (let j = 0; j < tileset.tileset_width; j++) {
          row.row_data.push({
            id: id_count,
            color: rgbToHex(
              imageData.data[id_count * 4],
              imageData.data[id_count * 4 + 1],
              imageData.data[id_count * 4 + 2]
            ),
          });
          id_count++;
        }
  ...
  const updateTilesetData = (id, color) => {
    tileset.tileset_data[0].data[
      Math.floor(id / tileset.tileset_height)
    ].row_data[id % tileset.tileset_width].color = color;
  };

  const fillUtil = (screen, x, y, color) => {
    if (
      x < 0 ||
      x >= tileset.tileset_width ||
      y < 0 ||
      y >= tileset.tileset_height
    ) {
      return;
    }
  ...
  const fill = (mPos) => {
    let cellid =
      Math.floor(mPos.y / zoomLevel) * tileset.tileset_width +
      Math.floor(mPos.x / zoomLevel);
    let color =
      tileset.tileset_data[0].data[Math.floor(mPos.y / zoomLevel)].row_data[
        Math.floor(mPos.x / zoomLevel)
      ].color;
    if (color === fillColor) {
      return;
  ...
  const handleMouseMove = (e) => {
    ...
    if (mouseDown === 1) {
      if (tool === "brush") {
        ...
        if (tile.color !== fillColor) setFillColor(tile.color);
      }
    }
